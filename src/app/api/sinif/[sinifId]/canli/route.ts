...
import { sinifCanliDurumu } from "@/lib/board-events";

export const dynamic = "force-dynamic";

/**
 * Tahtanın canlı yayınının yokladığı uç nokta. `sonrasi` verilen andan
 * sonraki kayıtları ve sınıfın O ANKİ aktif dersini döner; istemci dönen
 * `sonKontrol`u bir sonraki istekte `sonrasi` olarak geri gönderir.
 *
 * Ders id'si YANITTA gelir, istekte değil: tahta ders başlamadan açılmış
 * olabilir ve dersin başladığını başka türlü öğrenemez (bkz. board-events.ts).
 *
...
export async function GET(
  istek: Request,
  { params }: { params: Promise<{ sinifId: string }> },
) {
  ...
  const sonuc = await sinifCanliDurumu(sinifId, ogretmenId, sonrasi);
