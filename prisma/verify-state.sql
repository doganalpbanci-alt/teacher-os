...
WITH k AS (
  SELECT 1 s, 'Migration 1/4 · init' kontrol,
  ...
  UNION ALL SELECT 2, 'Migration 2/4 · koruma kurallari + RLS',
  ...
  UNION ALL SELECT 3, 'Migration 3/4 · davranis sablonu',
  ...
  UNION ALL SELECT 3.5, 'Migration 4/4 · teneffus cezasi',
  ...
  UNION ALL SELECT 4, 'Fazladan/taninmayan migration kaydi',
    (SELECT coalesce(string_agg(migration_name,', '),'yok') FROM "_prisma_migrations"
       WHERE migration_name NOT IN ('20260821214524_init','20260822105533_harden_history_and_rls',
                                    ...
                                    '20260823144543_break_penalty')), 'yok'
  ...
  UNION ALL SELECT 8, 'Gecmis korumasi (RESTRICT baglanti)',
    (SELECT count(*)::text FROM pg_constraint WHERE contype='f' AND confdeltype='r'), '11'
